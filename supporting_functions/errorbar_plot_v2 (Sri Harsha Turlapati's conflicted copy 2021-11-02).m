...
for subject_idx = 1 : length(real_V)
    ...
    text_h = text(real_K(subject_idx)+2.5,real_V(subject_idx),sprintf('$S_%d$',subject_idx),'Interpreter','Latex','fontsize',20);
    else
    text_h = text(ELM_K(subject_idx)+2.5,ELM_V(subject_idx),sprintf('$S_%d$',subject_idx),'Interpreter','Latex','fontsize',20);
    end
    ...
    title('Power-law characteristics spread - ELM vs real data')
