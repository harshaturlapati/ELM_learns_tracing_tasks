function [er_os_total] = min_lambda_C_v2(beta,arg_goal, ELM, N_train, Z_train_norm, ...
    Y_train_norm, pstar_all, Pt_all, GEO, Z_BUILDER, sampling_time, mean_Z, std_Z, mean_Y, std_Y)
...
%         inds = randperm(floor((N_train-100)*1/4), len_int);
    ...
    for k = 1:len_int
        ...
        W4 = (X_train(:,N_cur)*X_train(:,N_cur)'+ l * eye(Nx))\(X_train(:,N_cur) * Y_train_norm(:,N_cur)');
        b = mean(Y_train_norm(:,N_cur),2) - W4' * mean(X_train(:,N_cur), 2);
        Y_train_hat = W4' * X_train(:,N_cur) + b;
        %Y_train_hat = (Y_train_hat.*std_Y) + mean_Y;
        ...
        for h = 1:100
            ...
            Zt = (Zt - mean_Z)./std_Z;
            X_t_cur(:,h) = ELM.Dyn(zeros(ELM.Nx,1),Zt);
            Y_t_cur(:,h) = W4' * X_t_cur(:,h) + b;
            Y_t_cur_temp = (Y_t_cur(:,h).*std_Y) + mean_Y;
            Pt = Pt + GEO.MF(mod(pstar, 2 * pi)) * sampling_time * Y_t_cur_temp + beta * (GEO.C(pstar) - Pt);
            %Pt = Pt + GEO.MF(mod(pstar,2*pi))*sampling_time*Y_t_cur(:,h);
        end
        er_is_temp(k) = mean((mean((Y_train_hat-Y_train_norm(:,N_cur)).^2,2)));
        Y_test_cur = Y_train_norm(:,N_cur(end)+1:N_cur(end)+100);
        Y_test_hat = Y_t_cur;
