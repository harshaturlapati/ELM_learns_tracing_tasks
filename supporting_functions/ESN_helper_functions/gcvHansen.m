function [reg_min,G,reg_param] = gcvHansen(U,s,b,method)
...
smin_ratio = eps;                % Smallest regularization parameter.
...
if (strncmp(method,'Tikh',4) | strncmp(method,'tikh',4))
  ...
  reg_param(npoints) = min([s(p),s(1)*smin_ratio]);
  ...
  if (find_min)
    ...
    reg_min = fminbnd('gcvfun',...
      reg_param(min(minGi+5,npoints)),reg_param(max(minGi-5,1)),...
